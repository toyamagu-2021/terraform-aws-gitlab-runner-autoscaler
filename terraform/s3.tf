...
resource "aws_s3_object" "runner_dockerfile" {
  content = templatefile("${path.module}/tftpls/gitlab-runner-docker-machine.tftpl.dockerfile", {
  })
  key    = "gitlab-bootstrap/docker/gitlab-runner-docker-machine.dockerfile"
  bucket = aws_s3_bucket.runner_cache.bucket
}
