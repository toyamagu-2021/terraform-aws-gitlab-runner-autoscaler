...
// GitLab 
locals {
  gitlab_user_data = templatefile("./tftpls/install_docker_to_amzn_linux_2.tftpl.sh", {
    s3_bucket = {
  ...
  runner_config = {
    ...
    runner = {
      ...
      machine = {
        ...
        ec2_tags            = "runner-manager-name,gitlab-aws-autoscaler,gitlab,true,gitlab-runner-autoscale,true"
        security_group_name = aws_security_group.gitlab_runner.name
        instance_type       = var.gitlab_runner.instance_type
        instance_profile    = aws_iam_instance_profile.machine.name
      }
    }
  }
}
